...
function BoardList({ token }) {
  ...
  useEffect(() => {
    const fetchBoards = async () => {
      try {
        const response = await axios.get('//k8s-taskapp-taskappi-929be65fa1-941636230.ap-south-1.elb.amazonaws.com/api/boards', {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      const response = await axios.post(
        '//k8s-taskapp-taskappi-929be65fa1-941636230.ap-south-1.elb.amazonaws.com/api/boards',
        { title },
        { headers: { Authorization: `Bearer ${token}` } }
      );
