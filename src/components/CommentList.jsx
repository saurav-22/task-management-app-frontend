    ...
    const fetchComments = async () => {
      try {
        const response = await axios.get(
          `//k8s-taskapp-taskappi-929be65fa1-941636230.ap-south-1.elb.amazonaws.com/api/comments?taskId=${taskId}`,
  ...
  const handleCreate = async (e) => {
    ...
    try {
      const response = await axios.post(
        '//k8s-taskapp-taskappi-929be65fa1-941636230.ap-south-1.elb.amazonaws.com/api/comments',
        { content, taskId },
        { headers: { Authorization: `Bearer ${token}` } }
      );
